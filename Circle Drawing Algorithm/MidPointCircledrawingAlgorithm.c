...
void MidpointCircleDrawingAlgorithm(HDC hdc, int xc, int yc, int radius)
{
    int k = 0, pk, xk, yk;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    HWND hwnd = CreateWindow("MyWindowClass", "Mid Point Circle Drawing Algorithm", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 1000, NULL, NULL, hInstance, NULL);

    ShowWindow(hwnd, nCmdShow);
    UpdateWindow(hwnd);

    HDC hdc = GetDC(hwnd);
    MidpointCircleDrawingAlgorithm(hdc, xc, yc, radius);
    ReleaseDC(hwnd, hdc);
